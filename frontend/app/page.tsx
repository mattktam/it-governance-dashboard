...
export default function Home() {
  ...
  const topOwnersList = Object.entries(topOwners)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([owner, cost], idx) => ({
      rank: idx + 1,
  ...
  const topApplicationsList = Object.entries(topApplications)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([app, cost], idx) => ({
      rank: idx + 1,
  ...
  return (
    <main className="min-h-screen p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Top IT Owners and Applications */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          {/* Top 5 Spenders (IT Owners) */}
          <div className="bg-gradient-to-br from-card/50 to-card/30 border border-border/50 rounded-xl p-8 backdrop-blur-sm">
            <h2 className="text-2xl font-bold text-foreground mb-6">👥 Top 5 Spenders (IT Owners)</h2>
            <div className="space-y-3">
              {topOwnersList.map((owner, idx) => (
                <div
                  key={idx}
                  className={`flex items-center justify-between p-4 border border-border/50 rounded-lg bg-gradient-to-r ${gradients[idx]} hover:border-accent/50 transition-all hover:shadow-lg hover:shadow-purple-500/10 group`}
                >
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-foreground">#{owner.rank}</span>
                    <span className="font-medium text-foreground">{owner.name}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">YESTERDAY</p>
                      <p className="font-bold text-foreground">${owner.yesterday.toFixed(2)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">TODAY</p>
                      <p className="font-bold text-foreground">${owner.today.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          {/* Top 5 Application Jobs */}
          <div className="bg-gradient-to-br from-card/50 to-card/30 border border-border/50 rounded-xl p-8 backdrop-blur-sm">
            <h2 className="text-2xl font-bold text-foreground mb-6">💼 Top 5 Application Jobs</h2>
            <div className="space-y-3">
              {topApplicationsList.map((app, idx) => (
                <div
                  key={idx}
                  className={`flex items-center justify-between p-4 border border-border/50 rounded-lg bg-gradient-to-r ${gradients[idx]} hover:border-accent/50 transition-all hover:shadow-lg hover:shadow-purple-500/10 group`}
                >
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-foreground">#{app.rank}</span>
                    <span className="font-medium text-foreground">{app.name}</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">YESTERDAY</p>
                      <p className="font-bold text-foreground">${app.yesterday.toFixed(2)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">TODAY</p>
                      <p className="font-bold text-foreground">${app.today.toFixed(2)}</p>
                    </div>
                  </div>
                </div>
              ))}
